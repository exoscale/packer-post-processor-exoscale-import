...
// PostProcess takes an image artifact from the QEMU, Artifice, or File builders and imports it to Exoscale
func (p *PostProcessor) PostProcess(ctx context.Context, ui packer.Ui, a packer.Artifact) (packer.Artifact, bool, bool, error) {
	...
	default:
		err := fmt.Errorf(
			"unknown artifact type: %s, Can only import from QEMU/file builders and Artifice post-processor artifacts",
			a.BuilderId())
...
func (p *PostProcessor) registerTemplate(ctx context.Context, ui packer.Ui, url, md5sum string) (string, error) {
	...
	resp, err := exo.GetWithContext(ctx, &zone)
	if err != nil {
